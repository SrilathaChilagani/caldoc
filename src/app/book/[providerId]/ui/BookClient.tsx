...
export default function BookClient({ provider, slots, initialSlotId, initialMode }: Props) {
  ...
  const upcomingSlots = useMemo(
    () => slots.filter((slot) => new Date(slot.startsAt).getTime() >= Date.now()),
    [slots],
  );
  ...
  useEffect(() => {
    if (prefillName && !patientName) setPatientName(prefillName);
  }, [prefillName, patientName]);

  useEffect(() => {
    if (prefillPhone && !patientPhone) setPatientPhone(prefillPhone);
  ...
  function handlePage(direction: "prev" | "next") {
    setSlotIndex((prev) => {
      if (direction === "prev") return Math.max(0, prev - pageSize);
      const next = prev + pageSize;
      return Math.min(Math.max(0, upcomingSlots.length - pageSize), next);
    });
  }
  ...
  const compiledSymptoms = useMemo(() => {
    const base = selectedSymptoms.filter((s) => s !== "Other");
    ...
    try {
      ...
      const res = await fetch("/api/appointments/create", {
        ...
        body: JSON.stringify({
          ...
          visitMode,
          ...(bookingFor === "other" && bookerPhone.trim() ? { bookerPhone: bookerPhone.trim() } : {}),
        }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data?.error || "Could not create appointment");
      const amountQuery = data.amount ? `&amount=${data.amount}` : "";
      const embedQuery = isEmbed ? "&embed=1" : "";
      window.location.href = `/checkout?appointmentId=${data.appointmentId}${amountQuery}${embedQuery}`;
    } catch (err) {
    ...
    <div className="rounded-2xl border border-slate-200 bg-white p-5 mb-6">
      ...
      <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <div>
          ...
          <p className="text-xs font-mono text-slate-500">{patientPhone.trim()}</p>
        </div>
        <div>
